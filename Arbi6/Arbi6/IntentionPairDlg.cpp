...
#include "Info.h"
//*****business
#include "./arbi/TwoLegStrategy.h"
#include "./arbi/IntentionPair.h"
#include "./arbi/Persistence.h"
#include "./arbi/DayTradingStrategy.h"
...
BEGIN_MESSAGE_MAP(CIntentionPairDlg, CDialog)
	...
	ON_COMMAND(ID_INTENTION_ADD, &CIntentionPairDlg::OnAddItem)
 			...
 			for(it = iter->second->strategyInfoList.begin(); it != iter->second->strategyInfoList.end(); ++it)// list
 			{
 				...
 				if(ts1 != NULL){
 					if(ts1->isSecondLegMissed()) 
 					{
 						...
 						if(AfxMessageBox( _T("第二腿打空,请处理!"), MB_OK ) == IDOK)	
