...
Exchange * Exchange::sh = NULL;
Exchange * Exchange::sz = NULL;
...
void Exchange::initAll()
{
	...
	sh = new Exchange(SH);
	sh->add(new TimeInterval(9, 30, 0, 11, 30, 0));
	sh->add(new TimeInterval(13, 0, 0, 15, 0, 0));

	sz = new Exchange(SZ);
	sz->add(new TimeInterval(9, 30, 0, 11, 30, 0));
	sz->add(new TimeInterval(13, 0, 0, 15, 0, 0));
}
