...
{
	DialogSugar * dialogsugar = new DialogSugar();
	dialogsugar->Create(IDD_DIALOG1);
	dialogsugar->ShowWindow(SW_RESTORE);
	dialogsugar->CenterWindow(GetDesktopWindow());
}

void CMainFrame::OnStopBoHai() 
{
	((otherDealer *)Dealer::oversea)->redisStop();
}

void CMainFrame::OnStartBoHai()
{
	((otherDealer *)Dealer::oversea)->redisStart();
}

void CMainFrame::OnPauseBoHai()
{
	((otherDealer *)Dealer::oversea)->redisPause();
}
