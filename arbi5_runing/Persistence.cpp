...
string get_mac ()
{
    ...
    if ((dwRetVal = GetAdaptersInfo( pAdapterInfo, &ulOutBufLen)) == NO_ERROR)
    {
        ...
        {
            char mac[100];
            sprintf(mac, "%02x-%02x-%02x-%02x-%02x-%02x;", pAdapter->Address[0], pAdapter->Address[1], pAdapter->Address[2], pAdapter->Address[3], pAdapter->Address[4], pAdapter->Address[5]);
            all_mac += mac;
...
string todayFile()
{
    ...
    return intToString(tmp.tm_year + 1900) + intToString(tmp.tm_mon + 1) + intToString(tmp.tm_mday) + ".his";
}
